fzqblog.categoryUrl = {
    loadCategories: fzqblog.realpath + "/admin/loadBlogCategories.action",
    saveCategory: fzqblog.realpath + "/admin/saveBlogCategory.action",
    delCategory: fzqblog.realpath + "/admin/delBlogCategory.action"
};
$(function() {
    fzqblog.categories = $("#categories");
...
function delCategory(curObj) {
    ...
    layer.confirm('是否删除该分类？', {
        ...
        $.ajax({
            url: fzqblog.categoryUrl.delCategory,
            type: 'POST',
            dataType: 'json',
            data: {
...
function loadCategories() {
    fzqblog.categories.empty();

    $.ajax({
        url: fzqblog.categoryUrl.loadCategories,
        type: 'POST',
        dataType: 'json',
        data: {},
        success: function(res) {
            var list = res.data;
            if (list.length == 0) {
                $("<div class='no-data'>没有分类，点击新增分类添加吧。</div>").appendTo(fzqblog.categories);
            } else {
                for (var i = 0, _len = list.length; i < _len; i++) {
                    new CategoryItem(list[i]).appendTo(fzqblog.categories);
                }
            }
        }
    });
...
function addCategory(curObj) {
    ...
    var pid = 0;
    var children_panel;
    rank = fzqblog.categories.find(".category").length;
    var data = {
        name: "",
        categoryId: '',
        rank: rank + 1
    }
    if (fzqblog.categories.find(".no-data").length > 0) {
        fzqblog.categories.empty();
    }
    var category = new CategoryItem(data).appendTo(fzqblog.categories);
...
function saveCategory(curObj) {
    ...
    $.ajax({
        url: fzqblog.categoryUrl.saveCategory,
        type: 'POST',
        dataType: 'json',
        data: form.serialize(),
        success: function(res) {
