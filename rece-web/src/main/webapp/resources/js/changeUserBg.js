fzqblog.backgroundName = fzqblog.background.substring(fzqblog.background.lastIndexOf("/")+1);
fzqblog.userUrl={
	saveBackground:fzqblog.realpath+"/admin/saveSysUserBg"
}
$(function(){
...
function initSystemBackground(){
	for(var i=1;i<=10;i++){
		...
		$("<img src="+fzqblog.realpath+"/resources/images/user_bg/bg"+i+".jpg>").appendTo(system_icon);
		if(fzqblog.backgroundName==("bg"+i+".jpg")){
...
function saveUserBackground(curObj){
	...
	$.ajax({
		url : fzqblog.userUrl.saveBackground,
		type: 'POST',
		dataType: 'json',
		data : {
