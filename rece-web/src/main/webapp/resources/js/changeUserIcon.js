fzqblog.userIconUrl={
	saveCustomerUserIcon:fzqblog.realpath+"/admin/saveUserIcon",
	saveSysUserIcon:fzqblog.realpath+"/admin/saveSysUserIcon"
...
var uploader = WebUploader.create({
    ...
    // 文件接收服务端。
    server : fzqblog.realpath + '/imageUpload2Temp.action',
...
uploader.on('uploadSuccess', function(file, response) {
	...
	$("#imgarea").show();
	cutter.reload(fzqblog.realpath+"/upload/"+response.savePath);
    }
    $("#file_upload").hide();
    $("#uploading").hide();
});
...
function initSysUserIcon(){
	for(var i=1;i<=10;i++){
		...
		$("<img src="+fzqblog.realpath+"/resources/images/default_usericon/"+i+".jpg>").appendTo(system_icon);
	}
...
function saveUserIcon(){
	...
	var imgSavePath = imgPath.substring(start,data.s.lastIndexOf("?"));
		...
		$.ajax({
		url : fzqblog.userIconUrl.saveCustomerUserIcon,
		type: 'POST',
		dataType: 'json',
		data : {
		"img" : imgSavePath,
    ...
    else if(fzqblog.iconType==1){//系统头像
	...
	$.ajax({
		url: fzqblog.userIconUrl.saveSysUserIcon,
		type: 'POST',
		dataType: 'json',
