fzqblog.searchParam = "";
fzqblog.url = {
	loadMessage: fzqblog.realpath + "/admin/load_user_message_list.action",
	delMessage: fzqblog.realpath + "/admin/del_message.action",
	markMessage: fzqblog.realpath + "/admin/mark_message_read.action"
}
$(function() {
	demo(1);
...
function mark(messageId) {
	...
	var d = dialog({
		content: "<div><img src='" + fzqblog.realpath + "/resources/images/loading.gif' />&nbsp;&nbsp;&nbsp;标记中...</div>",
	});
	d.showModal();
	setTimeout(function() {
		d.close().remove();
...
function search() {
	fzqblog.searchParam = $("#searchform").serialize();
...
function demo(curr) {
	fzqblog.curr = curr;
	$.getJSON(fzqblog.url.loadMessage, {
		...
		if (list.length > 0) {
			for (var i = 0, _len = list.length, d; i < _len, d = list[i]; i++) {
				var edit = "";
				if (d.url != "") {
					edit = edit + '<a href="' + fzqblog.realpath + '/admin/readMessage.action?id=' + d.id + '" title="查看" target="_blank" class="read-btn" url="' + d.url + '" messageId="' + d.id + '"><i class="icon i-preview"></i>&nbsp;&nbsp;&nbsp;&nbsp;'
				}
