...
function addAsk(){
	...
	if(myMark < mark){
	 ...
	 var d = dialog({
			content: "<div><img src='" + fzqblog.realpath +"/resources/images/loading.gif' />&nbsp;&nbsp;&nbsp;发表中...</div>",
		});
		d.showModal();
		setTimeout(function() {
			d.close().remove();
		...
		$.ajax({
			url: fzqblog.realpath + '/ask/publicAsk',
			type: 'POST',
			dataType: 'json',
				...
				if(res.responseCode != "SUCCESS"){
					...
					layer.msg('发表成功', {
						  ...
						  time: 1500 //2秒关闭（如果不配置，默认是3秒）
							...
							document.location.href = fzqblog.realpath + "/ask/" + res.data ;
						});
				}
			}
		});
