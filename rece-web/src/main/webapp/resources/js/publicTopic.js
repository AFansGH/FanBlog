...
function topicTypeChoose(curObj) {
    ...
    if (fzqblog.topicType.topicType0 == topicType) {
    	$("#votePanel").hide();
    } 
    else if (fzqblog.topicType.topicType1 == topicType) {
    	$("#votePanel").show();
    }
}
...
function addTopic(){
	...
	if(fzqblog.topicType.topicType1 == topicType){
	...
	var numberReg = /^\d+$/;
	 ...
	 var d = dialog({
		...
		$.ajax({
			url: fzqblog.realpath + '/bbs/publicTopic',
			type: 'POST',
			dataType: 'json',
				...
				if(res.responseCode != "SUCCESS"){
					...
					layer.msg('发布成功,跳转到详情页', {
						  ...
						  time: 1500 //2秒关闭（如果不配置，默认是3秒）
							...
							document.location.href = fzqblog.realpath + "/bbs/" + res.data ;
						});
				}
			}
		});
...
$.ajax({
	url : fzqblog.realpath + "/bbs/loadCategories" ,
	type:"POST",
	dataType:"json",
	success : function(res) {
    var list = res.data;
    fzqblog.categories = list;
    for (var i = 0, _len = list.length; i < _len; i++) {
	var item = list[i];
