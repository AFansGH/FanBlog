...
function SearchItem(data){
	...
	var id = data.id;
	if(searchType=="T"){
		url = fzqblog.realpath+"/bbs/"+id;
	}else if(searchType=="K"){
		url = fzqblog.realpath+"/knowledge/"+id;
	}else if(searchType=="Z"){
		url = fzqblog.realpath+"/ask/"+id;
	}else if(searchType=="B"){
		url = fzqblog.realpath+"/user/"+data.userId+"/blog/"+id;
	}
	var item = $("<div class='search-item'></div>");
	$("<a href='"+url+"' target='_blank' class='search-item-title'>"+data.title+"</a><span class='time'>"+data.createTime+"</span><span class='search-by'>by</span><a class='search-user' target='_blank' href='"+fzqblog.realpath+"/user/"+data.userId+"'>"+data.userName+"</a>").appendTo(item);
