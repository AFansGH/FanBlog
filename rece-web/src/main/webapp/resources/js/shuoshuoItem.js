...
function ImageItem(img) {
	...
	$('<img src="'+ fzqblog.realpath  + '/upload/' + img + '" class="showimg">').appendTo(span);
	$('<a href="javascript:void(0)"><img src="' + fzqblog.realpath + '/resources/images/icon/icon-del.png"></a>').appendTo(span);
	return span;
}
...
$(document).on("click", "#load-more", function() {
	...
	loadShuoShuos(fzqblog.pageNum + 1);
...
$(document).on("click", ".op-at", function() {
	...
	function doShuoShuoLike(curObj){
		...
		$.ajax({
			url: fzqblog.realpath + '/doShuoShuoLike',
			type: 'POST',
			dataType: 'json',
	...
	function publicShuoShuoComment(curObj){
		...
		$.ajax({
			url: fzqblog.realpath + '/publicShuoShuoComment',
			type: 'POST',
			dataType: 'json',
			data: {"shuoshuoId": shuoshuoId,
					"content" : content
	...
	function publicShuoShuo(){
		...
		$.ajax({
			url: fzqblog.realpath + '/publicShuoShuo',
			type: 'POST',
			dataType: 'json',
			data: {
				content: content,
	...
	function ShuoShuoItem(data){
		...
		$('<a href="../user/' + data.userId+ '"><img src="' + fzqblog.realpath + '/resources/images/'+ data.userIcon + '" class="cd-timeline-img img-circle"></a>').appendTo(item);
		...
		if (data.imageUrlSmall != null && data.imageUrlSmall != ""){
			...
			for (var i = 0, _len = imageSmallArray.length, img; i < _len, img = imageSmallArray[i]; i++){
				if (img !== ""){
					...
					$('<img src="' + fzqblog.realpath + '/upload/' + img + '" class="img-thumbnail" style="width: 150px;height: 100px;">').appendTo(userthumbnail);
				}
			}
		}
		...
		var userinfo = $('<a href="' + fzqblog.realpath + '/user/' + data.userId + '" class="user-info"><i class="glyphicon glyphicon-user"></i><span>' + data.userName + '</span></a>').appendTo(shuoshuoiteminfo);
	...
	function LikeItem(like){
		var likeItem = $('<a href="' + fzqblog.realpath + '/user/' + like.userId + '" class="like-info"><img class="user-like-info img-responsive" src="' + fzqblog.realpath + '/resources/images/' + like.userIcon + '"></a>');
		return likeItem;
	}
	function CommentItem(comment){
		...
		$('<a href="' + fzqblog.realpath + '/user/' + comment.userId + '"><img src="' + fzqblog.realpath + '/resources/images/' + comment.userIcon + '" class="user-icon-image img-circle"></a>').appendTo(commentusericon);
		var commentmain = $('<div class="comment-main"></a>').appendTo(commentitem);
		var commentcontent = $('<div class="comment-content"><a href="' + fzqblog.realpath + '/user/' + comment.userId + '">' + comment.userName + '</a>：' + comment.showContent +'</div>').appendTo(commentmain);
